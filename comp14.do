...
log using "C:\Users\WazX\Desktop\comp14", replace
*Abrindo a base de dado
 ...
 quietly gen ln_comp=ln(comparecimento)
 *Correlação
pwcorr ln_comp analfabetos theil pobreza ln_sau ln_edu desemprego ln_nas pib ln_num conc, star(.05)
...
quietly xtset codigo ano2
*COMPARECIMENTO* 
xtabond ln_comp analfabetos theil ln_sau desemprego ln_nas conc , lags(1) twostep artests(3) vce(robust)
...
*Teste de Sargan 
xtabond ln_comp analfabetos theil ln_sau desemprego ln_nas conc, lags(1) twostep artests(3)
estat sargan
****Teste de Brausch Pagan****
 xtset codigo ano
 *COMPARECIMENTO*
 xtreg ln_comp analfabetos theil theil ln_sau desemprego ln_nas conc, re vce(cluster codigo) theta
 xttest0
...
xtset codigo ano
 *COMPARECIMENTO*
xtreg ln_comp analfabetos theil ln_sau desemprego ln_nas conc, fe
*A rejeição da hipótese nula indica que parâmetros são diferentes entre indivíduos, desta forma FE é preferível à POLS.
****Teste de Hausmann*****
 *COMPARECIMENTO*
xtreg  ln_comp analfabetos theil ln_sau desemprego ln_nas conc, fe
estimates store fixed
xtreg  ln_comp analfabetos theil ln_sau desemprego ln_nas conc, re
hausman fixed
*A rejeição da hipótese nula indica que especificação correta é a de FE e não RE.
 *****Comparando regressões*****
	*COMPARECIMENTO*
	*Regressão OLS
 quietly regress  ln_comp analfabetos theil ln_sau desemprego ln_nas conc, vce(cluster codigo)
estimates store OLS 	
	*Regressão FE
 quietly xtreg  ln_comp analfabetos theil ln_sau desemprego ln_nas conc, fe vce(cluster codigo)
estimates store FE 
	*Regressão RE
 quietly xtreg  ln_comp analfabetos theil ln_sau desemprego ln_nas conc, re vce(cluster codigo)
estimates store RE 
	*Tabela de comparação
estimates table  OLS  FE  RE,  b  se  t  stats(N  r2  r2_o  r2_b  r2_w  sigma_usigma_e  rho theta)
    
save "C:\Users\WazX\Desktop\comp14", replace
translate @Results "C:\Users\WazX\Desktop\comp14.txt",replace
log close
