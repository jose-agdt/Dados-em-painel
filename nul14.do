...
*Abrindo a base de dado
 ...
 quietly gen ln_nul=ln(nulo)
  *Correlação
  pwcorr ln_nul analfabetos theil pobreza ln_sau ln_edu desemprego ln_nas pib ln_num conc, star(.05)
...
xtset codigo ano2
*Estimador Arellano-Bond
xtabond ln_nul analfabetos theil ln_sau desemprego ln_nas conc , lags(1) twostep artests(3) vce(robust)
*Teste de autocorrelação
estat abond, artests(3)
*Teste de Sargan 
xtabond ln_nul analfabetos theil ln_sau desemprego ln_nas conc, lags(1) twostep artests(3)
estat sargan
 ****Teste de Brausch Pagan****
  xtset codigo ano
 *NULO*
  xtreg ln_nul analfabetos theil theil ln_sau desemprego ln_nas conc, re vce(cluster codigo) theta
 xttest0
 ****Teste de Chow*****
	 xtset codigo ano
 *NULO*
xtreg ln_nul analfabetos theil ln_sau desemprego ln_nas conc, fe
****Teste de Hausmann*****
 *NULO*
xtreg  ln_nul analfabetos theil ln_sau desemprego ln_nas conc, fe
estimates store fixed
xtreg  ln_nul analfabetos theil ln_sau desemprego ln_nas conc, re
hausman fixed 
 *****Comparando regressões*****
    *NULO*
	*Regressão OLS
 quietly regress  ln_nul analfabetos theil ln_sau desemprego ln_nas conc, vce(cluster codigo)
estimates store OLS 	
	*Regressão FE
 quietly xtreg  ln_nul analfabetos theil ln_sau desemprego ln_nas conc, fe vce(cluster codigo)
estimates store FE 
	*Regressão RE
 quietly xtreg  ln_nul analfabetos theil ln_sau desemprego ln_nas conc, re vce(cluster codigo)
estimates store RE 
