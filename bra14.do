...
xtset codigo ano2
*Estimador Arellano-Bond
xtabond ln_bra analfabetos theil ln_sau desemprego ln_nas conc , lags(1) twostep artests(3) vce(robust)
*Teste de autocorrelação
estat abond, artests(3)
*Teste de Sargan 
xtabond ln_bra analfabetos theil ln_sau desemprego ln_nas conc, lags(1) twostep artests(3)
estat sargan
 ****Teste de Brausch Pagan****
 xtset codigo ano
 *BRANCO*
  xtreg ln_bra analfabetos theil theil ln_sau desemprego ln_nas conc, re vce(cluster codigo) theta
 xttest0
...
****Teste de Chow*****
	 xtset codigo ano
 *BRANCO*
xtreg ln_bra analfabetos theil ln_sau desemprego ln_nas conc, fe
*A rejeição da hipótese nula indica que parâmetros são diferentes entre indivíduos, desta forma FE é preferível à POLS.
****Teste de Hausmann*****
 *BRANCO*
xtreg  ln_bra analfabetos theil ln_sau desemprego ln_nas conc, fe
estimates store fixed
xtreg  ln_bra analfabetos theil ln_sau desemprego ln_nas conc, re
hausman fixed 
*****Comparando regressões*****
    *BRANCO*
	*Regressão OLS
 quietly regress  ln_bra analfabetos theil ln_sau desemprego ln_nas conc, vce(cluster codigo)
estimates store OLS 	
	*Regressão FE
 quietly xtreg  ln_bra analfabetos theil ln_sau desemprego ln_nas conc, fe vce(cluster codigo)
estimates store FE 
	*Regressão RE
 quietly xtreg  ln_bra analfabetos theil ln_sau desemprego ln_nas conc, re vce(cluster codigo)
estimates store RE 
	*Tabela de comparação
estimates table  OLS  FE  RE,  b  se  t  stats(N  r2  r2_o  r2_b  r2_w  sigma_usigma_e  rho theta)


  save "C:\Users\WazX\Desktop\bra14", replace
translate @Results "C:\Users\WazX\Desktop\bra14.txt",replace
log close
